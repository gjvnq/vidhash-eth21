...
contract HashDictionary {
	...
	function getEntryByPHash(bytes calldata hash) public view returns (Entry memory) {
		uint entry_idx = phash2idx[hash];
		require(entry_idx != 0, "entry does not exists");

		return entries_list[entry_idx];
	}

	/// @notice Gets the HashPair entry associated with a particular chash.
	function getEntryByCHash(bytes calldata hash) public view returns (Entry memory) {
	...
	function addEntry(HashPair calldata hpair) public {
		...
		phash2idx[hpair.phash] = entries_list.length;
		chash2idx[hpair.chash] = entries_list.length;
